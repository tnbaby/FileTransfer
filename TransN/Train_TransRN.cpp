...
vector<vector<double> > relation_graph_vec, relation_graph_tmp;
...
void run(int n_in,double rate_in,double margin_in,int method_in, double con_alpha_in)
{
    ...
    relation_graph_vec.resize(relation_num);
    ...
    relation_graph_tmp.resize(relation_num);
    for(int i=0; i<relation_tmp.size(); i++){
	relation_tmp[i].resize(n);
        relation_graph_tmp[i].resize(n);
    }
    ...
    for (int i=0; i<relation_num; i++)
    {
	    ...
	    relation_graph_vec[i][ii] = relation_vec[i][ii];	
	}
    }
    ...
    // Init from Node2vec
//    FILE* f_rgv = fopen((data_path+"relation_embed.txt").c_str(), "r");
//    int embed_num = 0;
//    int embed_dimension = 0;
//    if (fscanf(f_rgv, "%d", &embed_num) != 1)
//	cout << "relation graph embeded read error" << endl;
//    if (fscanf(f_rgv, "%d", &embed_dimension) != 1)
//	cout << "relation graph embeded dimension read error" << endl;
//    for(int i=0; i<embed_num; i++)
//    {
//	fscanf(f_rgv, "%s", buf);
//        for(int j=0; j<n; j++)
//		fscanf(f_rgv, "%lf", &relation_graph_vec[relation2id[buf]][j]);
//    }
//    fclose(f_rgv);
    bfgs();
}
...
void bfgs()
{
   ...
   for (int epoch=0; epoch<nepoch; epoch++)
   {
        ...
        for (int batch = 0; batch<nbatches; batch++)
        {
	    	...
	    	relation_graph_vec = relation_graph_tmp;
        }
        epoch_e = time(NULL);
        cout<<"epoch: "<<epoch<<" learning rate: "<<rate<<" res: "<<res<<" time: "<<(double)(epoch_e-epoch_s)<<endl;
        if(rate>0.0001 && (epoch+1)%10 == 0)
		rate *= (1-1.0*epoch/nepoch); // change  the learning rate as line did
        else if((epoch+1)%100 == 0)
		rate *= (1-1.0*epoch/nepoch);
        FILE* f2 = fopen(("relation2vec."+version).c_str(),"w");
        FILE* f3 = fopen(("entity2vec."+version).c_str(),"w");
	FILE* f4 = fopen(("relation_graph2vec."+version).c_str(),"w");
	FILE* f5 = fopen(("log.txt"),"a");
        if(f2 ==  NULL || f3 == NULL || f4 == NULL || f5 == NULL)
           cout << "file open failed" << endl;
	fprintf(f5, "epoch: %d\t leanring rate: %lf\t res: %lf\t time: %lf\n", epoch, rate, res, (double)(epoch_e-epoch_s));
        for (int i=0; i<relation_num; i++)
        {
            for (int ii=0; ii<n; ii++)
	...
	{
	    for (int ii=0; ii<n; ii++)
		fprintf(f4, "%.6lf\t", relation_graph_vec[i][ii]);
	    fprintf(f4, "\n");
	}
        fclose(f2);
        fclose(f3);
...
double get_sum(int rel2, int rel1, int tid)
{
    double sum=0;
    if (L1_flag)
    	for (int ii=0; ii<n; ii++)
...
void get_neighbors_attention_vec(int rel, int tid)
{
    if(relation_neighbors.count(rel) == 0)
	return ;
    vector<double> weight(relation_neighbors[rel].size());
    double sum = 0;
    for (int i = 0; i<relation_neighbors[rel].size(); i++)
    {
    	sum += exp(-get_sum(relation_neighbors[rel][i], rel, tid));
    	weight.push_back(exp(-get_sum(relation_neighbors[rel][i], rel, tid)));
    }
    for (int ii=0; ii<n; ii++)
    {
    	relation_graph_vec[rel][ii] = 0.0;
    	for (int i=0; i<relation_neighbors[rel].size(); i++)
    		relation_graph_vec[rel][ii] += weight[i]/sum*relation_vec[relation_neighbors[rel][i]][ii];
    }
    norm(relation_graph_vec[rel]);
}
    
double calc_sum(int e1,int e2,int rel, int tid)
{
    double sum = 0;
    get_neighbors_attention_vec(rel, tid);
    get_neighbors_attention_vec(rel, tid);
    if (L1_flag)
    	for (int ii=0; ii<n; ii++)
...
void gradient(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b, int tid)
{
    for (int ii=0; ii<n; ii++)
    {
        double x = 2*(entity_vec[e2_a][ii]-entity_vec[e1_a][ii]-relation_vec[rel_a][ii]*(1-con_alpha)-con_alpha*relation_graph_vec[rel_a][ii]);
        if (L1_flag)
        	if (x>0)
        		x=1;
        	else
        		x=-1;
        relation_tmp[rel_a][ii]-=-1*rate*x*(1-con_alpha);
        entity_tmp[e1_a][ii]-=-1*rate*x;
        entity_tmp[e2_a][ii]+=-1*rate*x;
        x = 2*(entity_vec[e2_b][ii]-entity_vec[e1_b][ii]-relation_vec[rel_b][ii]*(1-con_alpha)-con_alpha*relation_graph_vec[rel_b][ii]);
        if (L1_flag)
        	if (x>0)
        		x=1;
        	else
        		x=-1;
        relation_tmp[rel_b][ii]-=rate*x*(1-con_alpha);
